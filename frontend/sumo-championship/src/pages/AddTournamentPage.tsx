...
const AddTournament = () => {
    ...
    useEffect(() => {
        const getCategories = async () => {
            const categories = await getCategoriesForSeason(generalInformation.seasonName)
    ...
    const handleAddTournament = async () => {
        ...
        if(generalInformation.registerEnd < generalInformation.registerStart){
            isSuccesful = false;
            setGeneralInformationErrors(prev => ({...prev, registerStart: "Start of registration must be before end"}))
        }
        ...
        if (generalInformation.contestStart.trim() === ""){
            isSuccesful = false;
            setGeneralInformationErrors(prev => ({...prev, contestStart: "You need to provide a Start of contest date"}))
        }

        if (generalInformation.contestEnd.trim() === ""){
            isSuccesful = false;
            setGeneralInformationErrors(prev => ({...prev, contestEnd: "You need to provide a end of contest date"}))
        }

        if (generalInformation.country.trim() === ""){

            isSuccesful = false;
            setGeneralInformationErrors(prev => ({...prev, country: "You need to provide a country"}))
        }

        if (generalInformation.city.trim() === ""){
            isSuccesful = false;
            setGeneralInformationErrors(prev => ({...prev, city: "You need to provide a city"}))
        }

        if (generalInformation.street.trim() === ""){
            isSuccesful = false;
            setGeneralInformationErrors(prev => ({...prev, street: "You need to provide a street"}))
        }

        if (generalInformation.nr === undefined){
            isSuccesful = false;
            setGeneralInformationErrors(prev => ({...prev, nr: "You need to provide a street number"}))
        }

        if (generalInformation.registerEnd.trim() === ""){
            isSuccesful = false;
            setGeneralInformationErrors(prev => ({...prev, registerEnd: "You need to provide a end of register date"}))
        }

        if (generalInformation.registerStart.trim() === ""){
            isSuccesful = false;
            setGeneralInformationErrors(prev => ({...prev, registerStart: "You need to provide a start of register date"}))
        }
